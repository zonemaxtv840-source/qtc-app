...
def generar_excel_web(items, cliente, ruc):
    ...
    fmt_h = workbook.add_format({'bg_color': '#F79646', 'bold': True, 'font_color': 'white', 'border': 1, 'align': 'center'})
...
if f_p and f_s:
    ...
    precios_opc = [c for c in df_p.columns if any(p in str(c).upper() for p in ['VIP', 'P.', 'BOX', 'MAYOR', 'IR', 'SUGERIDO'])]
    col_p_sel = st.selectbox("🎯 Precio a aplicar:", options=precios_opc)

    st.divider()
    txt_area = st.text_area("⌨️ Pega SKU:CANTIDAD (ej: CN1271072NA8:4, CN0900374NA8:6)")
    ...
    if st.button("🚀 PROCESAR DISPONIBILIDAD"):
        ...
        for sku_ped, cant_ped in pedido_final.items():
            ...
            for _, fila in variantes.iterrows():
                ...
                # Usar iloc[0] para evitar error de múltiples filas
        ...
        if res:
            ...
            with st.expander("📥 Generar Cotización Excel"):
                ...
                if not items_ok.empty:
                    ...
                    st.download_button(label="📥 Descargar Excel", data=generar_excel_web(final_list, n_cli, "-"), file_name=f"Cotizacion_{n_cli}.xlsx")
