...
if "carrito_buscar" not in st.session_state: st.session_state.carrito_buscar = {}
...
with tab_cotizacion:
    if not st.session_state.catalogos:
        ...
        # ✅ Transferencia segura con .pop()
        skus_transferidos = st.session_state.pop('skus_transferidos', {})
        texto_defecto = "\n".join([f"{sku}:{cant}" for sku, cant in skus_transferidos.items()]) if skus_transferidos else ""
...
with tab_buscar:
    ...
    if not st.session_state.catalogos:
            ...
            with st.spinner("🔍 Buscando..."):
                ...
                for cat in st.session_state.catalogos:
                    ...
                    for _, row in df[mask].iterrows():
                        ...
                        stock_total, _, stock_apri004, stock_yes, stock_col = buscar_stock(st.session_state.stocks, sku, st.session_state.tipo_cotizacion)
                        
                        resultados_busqueda.append({
                            'SKU': sku, 'Descripcion': str(row[cat['col_desc']])[:100],
                            'Precio': precio, 'Stock_Total': stock_total,
                            'Stock_Detalle': stock_col
                        })
        ...
        if st.session_state.carrito_buscar:
            ...
            st.dataframe(pd.DataFrame([{'SKU': k, 'Cantidad': v} for k, v in st.session_state.carrito_buscar.items()]), use_container_width=True)
