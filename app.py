...
if "GOOGLE_API_KEY" in st.secrets:
    ...
    try:
        ...
        model = genai.GenerativeModel('gemini-pro-vision') # Fallback por si acaso
else:
...
def extraer_codigos_ia(imagen_pil):
    prompt = """
    Analiza la imagen y extrae todos los códigos de producto (ej: CN0900033NA8, RN0800070NA8, WH0...).
    Formato: SKU:CANTIDAD, separado por comas.
    Si no hay cantidad, pon 1.
    Ejemplo: CN0900033NA8:1, RN0800070NA8:2
    SOLO responde los códigos, nada de texto extra.
    """
    try:
        # El motor de Google necesita que la imagen esté en bytes para algunas versiones
...
if f_p and f_s:
    ...
    def auto_clean(df):
        for i in range(min(15, len(df))):
            ...
            if any(h in val for h in ['SKU', 'SAP', 'COD SAP', 'ARTICULO'] for val in row_vals):
                df.columns = [str(c).strip() for c in df.iloc[i]]
                return df.iloc[i+1:].reset_index(drop=True)
        return df

    df_p = auto_clean(df_p_raw)
    df_s = auto_clean(df_s_raw)
    ...
    if img_file:
        ...
        if st.button("🔍 ESCANEAR CON IA"):
            with st.spinner("IA analizando imagen..."):
                ...
                if "ERROR_API" in res_raw:
                    st.error(f"Error de conexión: {res_raw}")
                else:
                    st.info(f"🤖 IA leyó: {res_raw}")
                    temp = {}
                    for it in res_raw.replace('`', '').split(','):
                        if ':' in it:
                            s, c = it.split(':')
                            temp[s.strip().upper()] = int(re.sub(r'\D', '', c)) if c.strip() else 1
                        else:
    ...
    if st.button("🚀 PROCESAR DISPONIBILIDAD") and st.session_state.pedido_ia:
        ...
        if final_res:
            df_final = pd.DataFrame(final_res).drop_duplicates()
            st.subheader("📊 Resultados de Stock")
            def color_r(row): return ['background-color: #FF3333; color: black; font-weight: bold' if row.ALERTA == "SIN STOCK" else '' for _ in row]
            st.dataframe(df_final[[c_sku_p, c_desc_p, 'P_UNIT', 'PEDIDO', 'Disp', 'ALERTA']].style.apply(color_r, axis=1))
